...
const generateToken = (user) => {
 ...
 const registerForm = async (req, res) => {
  try {
    ...
    const existingUser = await AptitudeUser.findOne({ email });
    if (existingUser) {
...
const otpVerify = async (req, res) => {
  try {
    ...
    const existingUser = await AptitudeUser.findOne({ email: decoded.user.email });
    if (existingUser) {
      return res.status(400).json({ message: "User already exists" });
    }

    const user = await AptitudeUser.create({
...
const login = async(req,res)=>{
  ...
  try {
    ...
    const accessToken = jwt.sign({userId: user._id}, process.env.SECRET_CODE, { expiresIn: "15m" });
...
const forgotPassword = async (req, res) => {
    ...
    const resetPassword = async (req, res) => {
        ...
        try {
            const decoded = jwt.verify(token, process.env.SECRET_CODE); 
            const user = await AptitudeUser.findOne({ email: decoded.email });
...
module.exports = {registerForm, otpVerify, login, forgotPassword,verifyOtp, resetPassword,refreshToken};